...
function SearchResultsContent() {
  ...
  // Fetch search results — runs only when query or page changes
  useEffect(() => {
    if (!query) {
      setResults([]);
      setTotalResults(0);
    ...
    const fetchResults = async () => {
      setLoading(true);
      try {
        const controller = new AbortController(); // Allows cleanup on unmount
  ...
  // Safe pagination update using client-side navigation
  const goToPage = (page: number) => {
    if (page < 1 || page > totalPages || page === currentPage) return;

    const params = new URLSearchParams(searchParams.toString());
    params.set("page", page.toString());
    window.location.search = params.toString(); // Triggers re-render safely
  };
                      ...
                      <div className="p-5">
                        ...
                        {(product.brand || product.category) && (
                          <p className="text-sm text-gray-500 mb-2">
                            {product.brand?.name} {product.brand && product.category && "• "}
              ...
              <div className="flex justify-center items-center gap-3 mt-16">
                ...
                <div className="flex gap-2">
                  {visiblePages().map((page, i) =>
                    page === "..." ? (
                      <span key={i} className="px-4 py-3 text-gray-500">...</span>
                    ) : (
                      <button
                        key={page}
